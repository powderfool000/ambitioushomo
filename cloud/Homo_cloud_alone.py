...
def handshake():
    ...
    sta = Peer('abc1238', my_mac, 'STA')
    #ap = Peer('server', my_mac, 'AP')
    ...
    #scalar_element_sta = json.dumps({"a": scalar_sta, "b": element_sta})
    #sock.send(scalar_element_sta.encode())

    print()
    logger.info('Computing shared secret...\n')
    
    '''
    scalar_element_ap = sock.recv(1024)
    scalar_element_ap = json.loads(scalar_element_ap.decode())
    scalar_ap = int(scalar_element_ap.get("a"))
    element_ap = scalar_element_ap.get("b")
    '''

    scalar_element_ap = sock.recv(1024).decode()
    data = scalar_element_ap.split('\n')

    #scalar_ap = int(sock.recv(1024).decode())
    ...
    #cloudfilesize = int(sock.recv(1024).decode())
    #receivedsize = 0
    ...
    with open(cloud_data, 'wb') as fw:
        ...
        fw.close()
    '''
    with open('cloud.data', 'wb') as file:
        print ('file opened')
        while True:
    ...
    #print('cloud file size', str(cloudfilesize))
    #print("Size of received file is : ", str(receivedsize),'\n')
    print('Successfully received the cloud.data file of size \n' , os.path.getsize('cloud.data'), '\n')
    
    subprocess.call("./testadd_cloud")
    
    answer_data_file = "answer.data"
    unencrypted_answerdata_filesize = os.path.getsize(answer_data_file)

    print("Size of answer file is : ", str(unencrypted_answerdata_filesize),'\n')
    print('sending answer.data..........')
    
    sock.send(str(unencrypted_answerdata_filesize).encode('utf-8'))

    with open(answer_data_file, 'rb') as fs:
    ...
    #print('cloud file size', str(cloudfilesize))
    #print("Size of received file is : ", str(receivedsize),'\n')
    print('\n cloud.data file of size : \n' , os.path.getsize('cloud.data'), '\n')
    print("\nthe initial answer.data file size is : \n ", unencrypted_answerdata_filesize)
    #print("the encrypted file size is \n ", os.path.getsize(output_answerdata_file))
    
    sock.close()
    print('\n cloud.data file MD5 : ')
    os.system("md5sum cloud.data")
    print('\n answer.data file MD5 : ')

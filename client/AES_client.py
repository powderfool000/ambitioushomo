...
def handshake():
    ...
    with open('secret.key.hacklab', 'wb') as f:
        print ('file opened')
        while True:
    ...
    print('Successfully get the file of size', os.path.getsize('secret.key.hacklab'))
    

    '''
    f = open("hahaha.hacklab",'wb') #open in binary
    #while (True):       
    ...
    decrypted = decrypt(ciphertext, PMK_key)
    print('the secret is',bytes.decode(decrypted))

    '''
    print("decrypting the file \n")
    secret_key_file = decrypt(PMK_key, 'secret.key.hacklab')

    print("acquired secret key file size\n", os.path.getsize(secret_key_file))

...
def handshake():
    ...
    with conn:
        ...
        #complie the C++ file first
        subprocess.call("./testadder")
        unencrypted_secretkey_filesize = os.path.getsize("secret.key")

        #run the C++ Adder_alice file
        ...
        encryptingHEkeysstart = time.time()
        output_secret_key_file = encrypt(PMK_key, secret_key_file)
        encryptingHEkeysend = time.time()

        #encrypted = encrypt("This is a secret message", PMK_key)
        print("this file " , output_secret_key_file,  " is encrypted \n ")
        print("the file size is \n ", os.path.getsize(output_secret_key_file))
        
        sendingHEkeystart = time.time()
        
        f = open(output_secret_key_file,'rb') # Open in binary
        l = f.read(8192)
        while (l):
            conn.send(l)
            #print('Sent ',repr(l))
        ...
        sendingHEkeyend = time.time()

        print("\nthe initial file size is \n ", unencrypted_secretkey_filesize)
        ...
        encryptingkeytime = encryptingHEkeysend - encryptingHEkeysstart
        sendprikeytime = sendingHEkeyend - sendingHEkeystart

        timelist = [totalkeysendtime, dragonflykeyauthtime, encryptingkeytime, sendprikeytime]

        keytimefile = "keysendtime.csv"

        if os.path.isfile(keytimefile):
            print("Appending to file keysendtime.csv")
            with open('keysendtime.csv','a') as file:
                writer = csv.writer(file)
                writer.writerow(timelist)              
        else:
            print("Making new file keysendtime.csv")
            with open('keysendtime.csv','w') as file:
                fields=['totalkeysendtime','dragonflykeyauthtime','encryptingkeytime','sendprikeytime']
                writer = csv.writer(file)
        ...
        '''
        might want to take a look at importing these modules:
        import openpyxl
        import Workbook
        ...
        print("Encrypting key time: ", encryptingHEkeysend - encryptingHEkeysstart)
        print("Sending Key time: ", sendingHEkeyend - sendingHEkeystart)

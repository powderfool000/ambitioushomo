...
def handshake():
    ...
    with conn:
        ...
        #encrypting ciphertexts
        cloud_data = "cloud.data"
        unencrypted_clouddata_filesize = os.path.getsize("cloud.data")
                
        print("\n sending the ciphertext file")
        
        conn.send(str(unencrypted_clouddata_filesize).encode('utf-8'))


        BUFFER_SIZE = 4096
        with open(cloud_data, 'rb') as fs:
        ...
        print("\nthe cloud.data file size is ", unencrypted_clouddata_filesize)
        
        answer_data_file = "answer.data"

        with open(answer_data_file, 'wb') as fw:
        ...
        '''
        answerdatafilesize = int(conn.recv(1024).decode())
        print("the received answerdata file size is : \n ", str(answerdatafilesize),'\n')
        
        #receive answer.data file from the cloud
        print("receiving the file......... \n")
        receivedsize = 0
        with open('answer.data', 'wb') as file:
            print ('file opened')
            while True:
                ...
                if receivedsize >= answerdatafilesize :
                #if not data:
                    print('data size reached \n') 
                    break   
        ...
        print("\nthe sent cloud.data file size is : ", unencrypted_clouddata_filesize)

        #print("the received answerdata file size is : \n ", str(answerdatafilesize),'\n')
        #print('received file loop size:', str(receivedsize),'\n')
        print('Successfully get the answer.data file of size : ', os.path.getsize('answer.data'),'\n')
        
        print("decrypting the file .................. \n")
        #answer_data_file = decrypt(PMK_key, 'answer.data.hacklab')
        #print("answer.data file size", os.path.getsize(answer_data_file))
